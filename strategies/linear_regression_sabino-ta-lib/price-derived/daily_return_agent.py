...
Simple arithmetic daily (bar-to-bar) return:
...
--------
* r          (raw return)
* r_zscore   (z-score of the last 20 returns)
"""
...
class DailyReturn_Agent:
    ...
    def _feat(self, df):
        ...
        r_z = (r - r.rolling(self.z).mean()) / r.rolling(self.z).std()
        d = df.copy()
        d["r"] = r
        d["r_z"] = r_z
        return d.dropna(subset=["r", "r_z"])

    # ── training ─────────────────────────────────────
    def fit(self, df):
        d = self._feat(df)
        if len(d) < self.z + 10:
            raise ValueError("Not enough rows for DailyReturn_Agent")
        X, y = d[["r", "r_z"]][:-1], (d["close"].shift(-1) > d["close"]).astype(int)[:-1]
    ...
    def predict(self, *, current_price, historical_df):
        ...
        last = self._feat(historical_df).iloc[-1:][["r", "r_z"]]
        return 2 * self.m.predict_proba(last)[0, 1] - 1

    def strategy(self, historical_df: pd.DataFrame) -> float:
        """
        Generate a trading signal using Daily Return.
        ...
        try:
            ...
            print(f"Error in Daily Return strategy: {str(e)}")
            return 0.0000
