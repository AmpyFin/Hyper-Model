...
%R = (highest-H − close) / (highest-H − lowest-L) × (-100)

Features
--------
* wr_norm   = -%R / 100    (0‒1, where 1 = overbought)
* wr_slope
"""
...
class WilliamsR_Agent:
    def __init__(self, length: int = 14):
        self.n = length
        self.m = LogisticRegression(max_iter=1000); self.fitted = False

    def _feat(self, df):
        highest = df["high"].rolling(self.n).max()
        lowest = df["low"].rolling(self.n).min()
        wr = -100 * (highest - df["close"]) / (highest - lowest)
        d = df.copy()
        d["wr_norm"] = -wr / 100.0        # 0‒1
        d["wr_slope"] = d["wr_norm"].diff()
        return d.dropna(subset=["wr_norm", "wr_slope"])

    def fit(self, df):
        d = self._feat(df)
        if len(d) < self.n + 10:
            raise ValueError("Not enough rows for WilliamsR_Agent")
        X, y = d[["wr_norm", "wr_slope"]][:-1], (d["close"].shift(-1) > d["close"]).astype(int)[:-1]
        self.m.fit(X, y); self.fitted = True

    def predict(self, *, current_price, historical_df):
        if not self.fitted:
            self.fit(historical_df)
        last = self._feat(historical_df).iloc[-1:][["wr_norm", "wr_slope"]]
        return 2 * self.m.predict_proba(last)[0, 1] - 1

    def strategy(self, historical_df: pd.DataFrame) -> float:
        """
        Generate a trading signal using Williams %R.
        ...
        try:
            ...
            print(f"Error in Williams %R strategy: {str(e)}")
            return 0.0000
