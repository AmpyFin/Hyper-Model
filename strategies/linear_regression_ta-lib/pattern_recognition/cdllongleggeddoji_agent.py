...
def _flag(df):
    ...
    rng = (h - l).replace(0, np.nan)  # Replace zeros with NaN
    body = abs(c - o)
    upper = h - c.where(c > o, o)
    lower = o.where(c > o, c) - l
    
    # Handle each condition separately with NA handling
    body_small = (body / rng <= 0.1).fillna(False)
    upper_big = (upper >= rng * 0.4).fillna(False)
    lower_big = (lower >= rng * 0.4).fillna(False)
    
    cond = body_small & upper_big & lower_big
