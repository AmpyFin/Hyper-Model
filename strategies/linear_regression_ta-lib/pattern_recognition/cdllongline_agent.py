...
Long Line Candle (single-bar):

* Body ≥ 60 % of total range.
* Upper & lower shadows ≤ 10 % of range.
...
def _flag(df):
    ...
    cond = (body / rng >= 0.6) & (upper / rng <= 0.1) & (lower / rng <= 0.1)
    return cond.shift().fillna(0).astype(float)


class CDLLONGLINE_Agent:
    def __init__(self):
    ...
    def fit(self, df):
        d = self._feat(df)
        if len(d) < 30: raise ValueError("Not enough rows for LONGLINE")
