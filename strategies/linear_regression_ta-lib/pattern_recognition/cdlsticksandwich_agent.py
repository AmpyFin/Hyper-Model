...
def _long(i, df):  
    body = _body(i, df)
    rng = _range(i, df)
    return (body >= rng * 0.5).fillna(False)
...
def _flag(df):
    ...
    black2 = (c.shift(2) < o.shift(2)).fillna(False) & _long(2, df)
    white1 = (c.shift(1) > o.shift(1)).fillna(False)
    close_equal = ((c - c.shift(2)).abs() <= tick).fillna(False)
    black0 = (c < o).fillna(False)
    
    # Combine conditions with proper NA handling
    result = black2 & white1 & black0 & close_equal
