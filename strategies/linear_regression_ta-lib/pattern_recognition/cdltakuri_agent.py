...
* Lower shadow ≥ **3 × range** (i.e., long tail).
...
def _flag(df):
    ...
    cond = (body / rng <= 0.1) & (upper / rng <= 0.05) & (lower / rng >= 3.0)
    return cond.shift().fillna(0).astype(float)


class CDLTAKURI_Agent:
    ...
    def fit(self, df):
        d = self._feat(df)
        if len(d) < 30: raise ValueError("Not enough rows for TAKURI")
