...
class MEDPRICE_Agent:
    ...
    def _feat(self, df):
        ...
        mp = (df["high"] + df["low"]) / 2
        d = df.copy()
        d["div"] = (df["close"] - mp) / mp
        d["slope"] = mp.pct_change()
    ...
    def fit(self, df):
        d=self._feat(df)
        if len(d)<30: raise ValueError("Not enough rows for MEDPRICE_Agent")
